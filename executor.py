...
def gaussian(mean: float, std: float) -> None:
    ...
    print(result)
    LOGGER.info(
        "Gaussian fit results mean: %s with std: +/- %s",
        str(result.values[0]),
        str(result.errors[0]),
    )
...
def exponential(decay_constant: float) -> None:
    ...
    print(result)
    LOGGER.info(
        "exponential fit results decay constant: %s with error: +/- %s",
        str(result.values[0]),
        str(result.errors[0]),
    )
...
def linear(slope: float) -> None:
    ...
    print(result)
    LOGGER.info(
        "linear fit results slope: %s with error: +/- %s",
        str(result.values[0]),
        str(result.errors[0]),
    )
